...
if($getaction=="login"){
	...
	if(Multiuser=="1"){//允许注册
		if(empty($user_name) || empty($user_email) || empty($user_pass)){
			...
			if ($row = $results -> fetchArray()) {
				...
				$add_sql = "insert into ".TABLE."user (username, password, email, addtime, utime, salt) values ('$user_name', '$user_pass', '$user_email', '$addtime', '$addtime', '$salt')";
				$query = $conn->exec($add_sql);
				if($query){
					$success = "1";
					$error_code = "注册成功！";
					$gotourl = "login.php";
					$uid_sql = "select uid from ".TABLE."user where username='$user_name'";
					$uid_results = $conn->query($uid_sql);
					$row = $uid_results -> fetchArray();
					$uid = $row['uid'];
	...
	$user_email = post("user_email");
	if(!empty($_SESSION['error_times']) && $_SESSION['error_times']>=13){
		$error_code = "错误提示太多！";
		$gotourl = "login.php";
	}else{	
		if(empty($user_email)){
			...
			if($row = $results -> fetchArray()){
				...
				if($state==""){
					...
					$query = $conn->exec("update `".TABLE."user` set `utime`='$getpasstime' where uid='$uid'");
					$error_code = "邮件发送成功！";
					$success = "1";
					$gotourl = "login.php";
				}
